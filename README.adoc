...
== Converting a GitBub Repo

[source,bash]
----
curl -X POST "http://localhost:7202/converter/github/process" \
  -H "Content-Type: application/json" \
  -d '{
    "repoName": "owner/repository-name",
    "encryptedPat": "your-encrypted-personal-access-token",
    "name": "project-name",
    "gitUrl": "https://github.com/owner/repository-name.git",
    "branch": "main",
    "cssTheme: "https://www.abc.com/styles/abc.css"
  }'
----
